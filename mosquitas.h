...
struct bichos{
	...
	std::vector<std::vector<int> > tachos_por_manzana; 
	std::vector<int> NroTachos; 
	std::vector<float> N_descach; 
	...
	//****************************  Initial conditions ***************************************************************************
 	for(int i=0;i < N_;i++)
 	{
 		tacho[i] = i;			                              //to defined the bucket where the mosquito lives
			if(DISTRIBUCIONTACHOS == 1)				//to choose the buckets distribution per block
			{
				manzana_del_tacho[tacho[i]] = int (i/5);	//fixed number of buckets
			}		
			else{
			manzana_del_tacho[tacho[i]] = int(ran2(&semilla)*NUMEROMANZANAS);//for random buckets per block
			}  
			
		manzana[i] = manzana_del_tacho[tacho[i]];                	//to defined the block for the bucket
		tachos_por_manzana[manzana[i]].push_back(tacho[i]);      	//to identify the bucket in the block    	
		int j = tacho[i];
		Tau[j] = 0;   						   	//Bucket disponibility (available, i.e., 0)
		int tachosxmanzana = tachos_por_manzana[manzana[i]].size();	//to identify the number of buckets per block
		
			if(tachosxmanzana <= 9)//conditional for a maximun number of buckets per block
			{ 
				estado[i] = ESTADOVIVO; 			//mosquito state: live (0)      		             
				edad[i] = ran2(&semilla)*7+19;		//mosquito age 	              
				pupacion[i] = tpupad-2+(ran2(&semilla)*5);  	//adulthood          
				TdV[i] = ran2(&semilla)*6+27;	             	//lifespan        
			}
	}

//***************** to check the buckets filled per block ************************

	int nmanzanas=tachos_por_manzana.size();

		for(int i=0;i < nmanzanas;i ++){
			int ntachos = tachos_por_manzana[i].size();   	//number of buckets per block
			int contTach = 0;                             	//counter for the number of buckets per block 

			for(int j = 0;j < ntachos;j ++)
			{
				//std::cout << (tachos_por_manzana[i])[j] << "\n";//to identify the bucket in the block
				contTach ++;
				NroTachos[i] = contTach;  
			}
		}
		
       //fill the array N_descach with the numbers of buckets to discard per block
...
/*Buckets transfer local (mosquitoes can move to another bucket within the same block) and global (mosquitoes can also move to another bucket of the first neighbouring block)*/
				...
				//check if the new bucket is saturated and available 
					...
					//if the bucket is saturated then looking for other located at the (new) block (avoid use GO TO)
					else 
					{
						int l=0;
